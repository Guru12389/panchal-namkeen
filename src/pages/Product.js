...
//   };
// useEffect(() => {
//   // Check if we came from cart with a product ID
//   if (location.state?.productId) {
//     const productId = location.state.productId;
//     // Find the product in the DOM and scroll to it
//     setTimeout(() => {
...
// // Add this to your product card
// // In the product grid mapping:
// <div 
...
export default function Product() {
  ...
  // Handle product highlight when coming from cart
  useEffect(() => {
    if (location.state?.productId) {
      const productId = location.state.productId;
      setTimeout(() => {
  ...
  const isInCart = (productId, weight) => {
    ...
    return cart.some((item) => item.cartId === cartId);
  };
  ...
  return (
    <div className="product-page">
      ...
      <section className="product-hero clean">
        <h1 className="fade-up delay-1">Our Products</h1>
        <p className="fade-up delay-2">Taste of Bharat in Every Bite • Aprakashi Foods</p>
      </section>
        ...
        {filteredProducts.length === 0 ? (
          ...
          filteredProducts.map((p) => (
            ...
            >
              ...
              <h3>{p.name}</h3>
              <p className="price">Starting From ₹{p.packs[0].price}</p>
              <p className="desc">{getSnippet(p.desc)}</p>
      ...
      {modalProduct && (
        <div className="modal-backdrop" onClick={() => setModalProduct(null)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <button className="close" onClick={() => setModalProduct(null)}>
              ×
            </button>
            ...
            <div className="pack-options">
              {modalProduct.packs.map((pack, index) => (
                <div
                  key={index}
                  className={`pack-card ${
                    selectedPack?.weight === pack.weight ? "selected" : ""
                  }`}
            ...
            {selectedPack && (
              <>
                <div className="selected-price">
                  Selected: <strong>{selectedPack.weight} — ₹{selectedPack.price}</strong>
                </div>
                ...
                {isInCart(modalProduct.id, selectedPack.weight) ? (
                  <div className="cart-actions-group">
                    ...
                    <button
                      className="btn remove-pack"
                      onClick={() => {
                        const cartId = `${modalProduct.id}-${selectedPack.weight}`;
                        removeFromCart(cartId);
                      }}
                    >
                      ❌ Remove
                  ...
                  <button
                    className="btn add"
                    onClick={() => handleAddToCart(modalProduct, selectedPack)}
                  >
                    🛒 Add To Cart
                  </button>
                )}
